...
const result = deduplicate('aabbcc', DeduplicateInputType.STRING);
  ...
  it('should throw an error if an unsupported input type is provided', () => {
    expect(() => deduplicate('test', 'invalid' as DeduplicateInputType)).toThrowError(
      'Unsupported type or input format for: invalid'
    );
  });

  it('should throw an error if input format does not match specified input type', () => {
    expect(() => deduplicate([1, 2, 3], DeduplicateInputType.STRING)).toThrowError(
      'Unsupported type or input format for: string'
    );
  });
});