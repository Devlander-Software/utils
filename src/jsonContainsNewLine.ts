...
export function jsonContainsNewLine(jsonInput: string | object): boolean {
  const jsonObject: Record<string, string | object> | boolean | Record<string, unknown> =
    isJson(jsonInput);
  if (!jsonObject) {
  ...
  if (typeof jsonObject !== 'boolean') {
    return checkForNewLine(jsonObject as unknown as Record<string, string | object>);
  } else {
